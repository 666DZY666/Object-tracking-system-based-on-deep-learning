...
*/
	...
	printf("eco_threshold: %f\r\n", params_.max_score_threshhold + eco_threshold);
...
*/
	...
	//bool train_tracker = frames_since_last_train_ >= (size_t)params_.train_gap;
        //bool train_tracker = frames_since_last_train_ >= 2;
	...
	params_.CG_opts.maxit = params_.CG_iter;
        ...
        //！！！！！！！！！！！！！！算法——跟蹤器更新方案修改！！！！！！！！！！！！！！
        if (scores.get_max_score() >= params_.max_score_threshhold + eco_threshold){
	    //bool train_tracker = frames_since_last_train_ >= (size_t)params_.train_gap;
            bool train_tracker = frames_since_last_train_ >= 6;
	    if (train_tracker)
	    {
	    ...
	    if (nScales_ > 0 && params_.use_scale_filter)
	    {
		    //scale_filter_ = scale_filter_.scale_filter_update();
	    }
        }
        else{
            frames_since_last_train_ = 0;
	...
	if (params_.debug == 1)
	{
		...
		cv::imshow("Object Tracking", resframe);
...
void ECO::init_features()
{
	...
	if (params_.useCnFeature && is_color_image_)//CN feature
	{
